...
fn yral_game_simulation_test() {
    ...
    // Forward timer by one slot (one slot is of 1 hr)
    ...
    // a user (charlie) can bet on a video only once.

    // Charlie places bet on Bob post 1 - again
    let charlie_place_bet_arg = PlaceBetArg {
        post_canister_id: bob_individual_template_canister_id,
        post_id: res1,
        bet_amount: 100,
    ...
    assert_eq!(bet_status, Err(BetOnCurrentlyViewingPostError::UserAlreadyParticipatedInThisPost));
    ic_cdk::println!("Bet status: {:?}", bet_status);
