...
use shared_utils::{canister_specific::{individual_user_template, platform_orchestrator::{self, types::args::PlatformOrchestratorInitArgs}, post_cache::types::arg::PostCacheInitArgs}, common::{types::{known_principal::{KnownPrincipalMap, KnownPrincipalType}, wasm::WasmType}, utils::system_time}, constant::{NNS_CYCLE_MINTING_CANISTER, NNS_LEDGER_CANISTER_ID}};
use test_utils::setup::test_constants::{get_global_super_admin_principal_id, get_mock_user_alice_canister_id, get_mock_user_alice_principal_id, v1::CANISTER_INITIAL_CYCLES_FOR_SPAWNING_CANISTERS};
...
fn when_subnet_known_principal_is_updated_it_is_reflected_in_individual_canisters() {
    ...
    pocket_ic.update_call(platform_canister_id, super_admin, "update_subnet_known_principal", candid::encode_args((subnet_orchestrator_canister_id, KnownPrincipalType::CanisterIdPostCache, post_cache_canister_id)).unwrap())
    .map(|res| {
    ...
    let post_cache_id_from_subnet = pocket_ic.query_call(subnet_orchestrator_canister_id, Principal::anonymous(), "get_well_known_principal_value", candid::encode_one(KnownPrincipalType::CanisterIdPostCache).unwrap())
    .map(|res| {
    ...
    let post_cache_id_from_individual = pocket_ic.query_call(alice_cannister_id, Principal::anonymous(), "get_well_known_principal_value", candid::encode_one(KnownPrincipalType::CanisterIdPostCache).unwrap())
    .map(|res| {
        let post_cache_id: Option<Principal> = match res {
            WasmResult::Reply(payload) => candid::decode_one(&payload).unwrap(),
