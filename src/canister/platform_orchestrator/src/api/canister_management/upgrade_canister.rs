...
use shared_utils::{
    ...
    constant::{
        ...
        POST_CACHE_CANISTER_CYCLES_THRESHOLD, 
...
async fn upgrade_individual_canisters(upgrade_arg: UpgradeCanisterArg) {
    ...
    CANISTER_DATA.with_borrow_mut(|canister_data| {
        ...
        canister_data.last_subnet_canister_upgrade_status.upgrade_arg = upgrade_arg.clone();
        canister_data.last_subnet_canister_upgrade_status.failures = vec![];
        canister_data.last_subnet_canister_upgrade_status.count = 0;
    });
    let subnet_orchestrator_canisters = CANISTER_DATA.with_borrow(|canister_data| canister_data.all_subnet_orchestrator_canisters_list.clone());
    ...
    for  subnet_orchestrator in subnet_orchestrator_canisters.iter() {
        match recharge_subnet_orchestrator_if_needed(*subnet_orchestrator).await {
            ...
            Err(e) => {
                CANISTER_DATA.with_borrow_mut(|canister_data| {
                    canister_data.last_subnet_canister_upgrade_status.failures.push(
                        (*subnet_orchestrator, e.to_string())
                    );
                });
                continue;
            }
...
async fn upgrade_subnet_canisters(upgrade_arg: UpgradeCanisterArg) {
    CANISTER_DATA.with_borrow_mut(|canister_data| {
        ...
        canister_data.wasms.insert(upgrade_arg.canister.clone(), canister_wasm);
    });
    ...
    let canister_list = CANISTER_DATA.with_borrow(|canister_data| {
        match upgrade_arg.canister {
            ...
            WasmType::SubnetOrchestratorWasm => Ok(canister_data.all_subnet_orchestrator_canisters_list.clone()),
            _ => Err(())
        }
        .unwrap()
    });
    ...
    let result_callback = |canister_upgrade_result: Result<Principal, (Principal, String)>| {
        match canister_upgrade_result {
            ...
            Err((canister_id, err)) => {
                CANISTER_DATA.with_borrow_mut(|canister_data| {
                    canister_data.last_subnet_canister_upgrade_status.failures.push((canister_id, err))
                });
            }
        }
    ...
    CANISTER_DATA.with_borrow_mut(|canister_data| {
        canister_data.subnet_canister_upgrade_log.append(&canister_data.last_subnet_canister_upgrade_status).expect("Could not write into subnet upgrade log");
    });

}


async fn recharge_and_upgrade_subnet_orchestrator(canister_id: Principal, upgrade_arg: UpgradeCanisterArg) -> Result<Principal, (Principal, String)>{

    match upgrade_arg.canister {
        WasmType::PostCacheWasm => {
            recharge_post_cache_canister_if_needed(canister_id).await.map_err(|e| (canister_id, e))?;
            upgrade_subnet_post_cache_canister(canister_id, upgrade_arg.wasm_blob, upgrade_arg.version).await.map_err(|e| (canister_id, e))?;
        }
        WasmType::SubnetOrchestratorWasm => {
            recharge_subnet_orchestrator_if_needed(canister_id).await.map_err(|e| (canister_id, e))?;
            upgrade_subnet_orchestrator_canister(canister_id, upgrade_arg.wasm_blob, upgrade_arg.version).await.map_err(|e| (canister_id, e))?;
        }
        _ => {}
    }
    Ok(canister_id)
...
async fn recharge_subnet_orchestrator_if_needed(canister_id: Principal) -> Result<(), String> {
    let (subnet_orchestrator_cycle_balance, ): (u128, ) = ic_cdk::call(
            canister_id,
...
async fn recharge_post_cache_canister_if_needed(canister_id: Principal) -> Result<(), String> {
    let (post_cache_canister_cycle_balance, ): (u128, ) = ic_cdk::call(
            canister_id,
            "get_cycle_balance", 
            ()
        )
