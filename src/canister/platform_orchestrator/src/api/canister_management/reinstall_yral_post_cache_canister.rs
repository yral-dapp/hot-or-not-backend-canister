...
use shared_utils::{
    ...
    constant::{GLOBAL_SUPER_ADMIN_USER_ID, YRAL_POST_CACHE_CANISTER_ID},
};
...
async fn reinstall_yral_post_cache_canister() {
    ...
    let canister_wasm = CANISTER_DATA
        .with_borrow_mut(|canister_data| canister_data.wasms.get(&WasmType::PostCacheWasm))
        .unwrap();
    ...
    known_principal_map.insert(
        ...
        Principal::from_text("74zq4-iqaaa-aaaam-ab53a-cai").unwrap(),
    );
    known_principal_map.insert(
