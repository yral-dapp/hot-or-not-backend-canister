...
use shared_utils::canister_specific::individual_user_template::types::{profile::{self, UserCanisterDetails}, session::SessionType};
...
async fn update_referrer_details(referrer: UserCanisterDetails) -> Result<String, String> {
    ...
    let profile_owner = CANISTER_DATA.with_borrow_mut(|canister_data| canister_data.profile.principal_id);
    if profile_owner.is_none() || !profile_owner.unwrap().eq(&caller()) {
        return Err("Unauthorized".into());
    }

    let (canister_session_type_result,): (Result<SessionType, String>, ) = call(referrer.user_canister_id, "get_session_type", ()).await.map_err(|e| e.1)?;
