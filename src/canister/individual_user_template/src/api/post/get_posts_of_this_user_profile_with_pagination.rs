...
fn get_posts_of_this_user_profile_with_pagination(
    ...
    CANISTER_DATA.with_borrow(|canister_data| {
        get_posts_of_this_user_profile_with_pagination_impl(from_inclusive_index, limit, canister_data, api_caller, current_time)
    })
}


fn get_posts_of_this_user_profile_with_pagination_impl(from_inclusive_index: u64, limit: u64, canister_data: &CanisterData, api_caller: Principal, current_time: SystemTime) -> Result<Vec<PostDetailsForFrontend>, GetPostsOfUserProfileError> {
...
mod test {
    ...
    use shared_utils::{canister_specific::individual_user_template::types::post::{FeedScore, Post, PostViewStatistics}, common::types::top_posts::post_score_index_item::PostStatus};

    use super::*;

    #[test]
    fn test_get_posts_of_this_user_profile_with_pagination_impl(
    ) {
        ...
        let all_created_posts: BTreeMap<u64, Post> = posts.into_iter().map(|post| (post.id, post)).collect();
        ...
        let result
            = super::get_posts_of_this_user_profile_with_pagination_impl(
                    0,
                    3,
                    &canister_data,
                    api_caller,
