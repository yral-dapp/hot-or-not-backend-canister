...
use super::get_profiles_that_follow_me_paginated::MAX_FOLLOW_ENTRIES_PER_PAGE;

#[ic_cdk::query]
#[candid::candid_method(query)]
fn get_profiles_i_follow_paginated(
    ...
    CANISTER_DATA.with(|canister_data_ref_cell| {
        ...
        get_profiles_i_follow_paginated_impl(&canister_data, last_index_received, &current_caller)
    })
}

fn get_profiles_i_follow_paginated_impl(
...
mod test {
    ...
    #[test]
    fn test_get_profiles_i_follow_paginated_impl() {
        let mut canister_data = CanisterData::default();
        let mut current_caller = Principal::anonymous();
        let mut last_index_received: Option<u64> = None;

        let result = get_profiles_i_follow_paginated_impl(&canister_data, None, &current_caller);
        ...
        let result = get_profiles_i_follow_paginated_impl(
            &canister_data,
            last_index_received,
            &current_caller,
        );
