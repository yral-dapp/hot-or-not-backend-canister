...
dfx canister install configuration --network ic --mode upgrade --argument "(record {})" --upgrade-unchanged
dfx canister install data_backup --network ic --mode upgrade --argument "(record {})" --upgrade-unchanged
dfx canister install post_cache --network ic --mode upgrade --argument "(record {})" --upgrade-unchanged
dfx canister install user_index --network ic --mode upgrade --argument "(record {})" --upgrade-unchanged
