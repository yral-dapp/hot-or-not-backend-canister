...
dfx canister install configuration --network ic --mode upgrade --argument "(record {})" --wasm target/wasm32-unknown-unknown/release/configuration.wasm.gz
dfx canister install data_backup --network ic --mode upgrade --argument "(record {})" --wasm target/wasm32-unknown-unknown/release/data_backup.wasm.gz
dfx canister install post_cache --network ic --mode upgrade --argument "(record {})" --wasm target/wasm32-unknown-unknown/release/post_cache.wasm.gz
dfx canister install user_index --network ic --mode upgrade --argument "(record {})" --wasm target/wasm32-unknown-unknown/release/user_index.wasm.gz
